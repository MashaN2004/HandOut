<?php
    ...
    $select = "SELECT * FROM posts WHERE user_id = '{$_SESSION['id']}'";
    $select2 = "SELECT * FROM users WHERE id = '{$_SESSION['id']}'";
    $select_query = mysqli_query($conn, $select);
    $users = mysqli_query($conn, $select2);
    $info = mysqli_fetch_assoc($users);
?>
...
<body>
  ...
  <main id="main">
        ...
        <div class="row gy-4">
          ...
          <div class="col-lg-8">
            <form action="insert.php" method="POST" enctype="multipart/form-data">
              ...
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Сылка</label>
                <input type="text" class="form-control" name="link" placeholder="Сылка">
              </div>
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Изображение</label>
                <input type="file" class="form-control" name="image" placeholder="Сылка">
              </div>
              <div class="mb-3">
                <label for="exampleFormControlTextarea1" class="form-label">Описание</label>
                <textarea type="text" class="form-control" name="text" rows="3"></textarea>
              </div>
              <button class="aad">Загрузить</button>
            </form>
          ...
          <div class="col-lg-4">
            <div class="portfolio-info">
              ...
              <ul style="font-size: 20px;">
                <li><strong>Имя</strong>: <?php echo $info["login"]?> </li>
                <li><strong>Email</strong>: <?php echo $info["email"]?></li>
                <li><strong>Password</strong>: <?php echo $info["password"]?></li>
              </ul>
            </div>
          </div>
    ...
    <section id="arc" class="services section-bg">
      <div class="container">
        ...
        <div class="row">
        <?
          for ($i=0; $i < mysqli_num_rows($select_query); $i++) { 
            $post = mysqli_fetch_assoc($select_query);
        ?>
  ...
  <footer id="footer">
    ...
    <div class="container footer-bottom clearfix">
      <div class="copyright">
        &copy; Copyright <strong><span>HandOut</span></strong>. All Rights Reserved
      </div>
      <div class="credits">
